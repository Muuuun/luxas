...
- **Iteration record (typed, 2026-08-25)**: every run's results.json carries `computed.iteration: {"headline_key": "computed.<key the acceptance criterion reads>", "headline_value": <that number>, "resolution": <the smallest change in it that could move the verdict — the criterion's tolerance, or the statistical error if larger>, "inherited_from": "run_<M>" | null, "kept": ["<pipeline / calibration / data / passing tests carried over>"], "changed": ["<what this run varied>"]}`. Two consecutive runs that move the headline by less than `resolution` surface to brain as a `<stopping_signal>` (ship / change hypothesis / change experiment space) — the epistemic stopping rule; a re-run with `inherited_from: null` after a run that produced a verdict is flagged `[REBUILD]` (the measured failure pattern is discarding what worked, not failing too often). For run_0, `inherited_from` is null and that is not a rebuild.
- **Anomaly record (typed, 2026-08-25)**: any result that deviates from your analytic-first prediction by more than its resolution (the >2× rule below is the coarse version) is recorded in `computed.anomalies: [{"observable": ..., "predicted": ..., "observed": ..., "affects_headline": true|false, "disposition": "pursued"|"explained"|"parked", "reason": "..."}]`. `parked` is legal ONLY when `affects_headline` is false; an anomaly that touches the headline is `pursued` (a run that tests it) or `explained` (a mechanism with evidence). Undispositioned entries re-surface to you and to brain every turn as `<undispositioned_anomalies>` — the surprise channel. An unexpected result is a goal ("explain this"), not a Limitations sentence.
