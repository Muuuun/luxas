...
- **Controls belong on the keys that will HEADLINE (2026-08-25).** The 297nm run validated intermediate anchors (d2 matrix element, a 38P benchmark) while the report's headline quantities (Ω/√P, the power budget) sat un-controlled at indicative. When you freeze the plan below, ask of each control: "is THIS the number the abstract will quote?" If the headline derives from an anchor, either put the control on the derived key too (one extra evaluation of the same script), or expose the derived quantity's encoding key (a fraction, a fitted exponent) and control that.
- **Name the control's route in one clause** (v3, 2026-08-29): "second-order perturbative channel sum" vs "full pair-Hamiltonian diagonalization"; "closed-form limit" vs "master equation". A second call of the same function, or the same formalism through another library, is not a control — the write-time validator flags `method_a`/`method_b` that share every content token, and the finish gate vetoes bit-identical values.
- **Cross-validation plan (run-or-demote, PERSISTED)**: for each quantity that will carry a headline claim, name the INDEPENDENT control method now — and write the plan into results.json in Phase 3 as `computed.cross_validation_plan: [{"claim_key": "<computed.* key>", "control_method": "<the control named here>"}]`. The finish gate requires every planned claim_key to be CLOSED: either an executed `cross_validation` entry, or a `method_blocked` entry carrying the control attempt's `failing_command` and `verbatim_last_error`. Declaring the control infeasible in prose closes nothing — "I could not run the control" is a capability claim and needs its verbatim error like any other (observed 2026-07-14: an experiment called the prescribed full-diagonalization control "memory-limited" in prose, wrote a phantom `method_blocked[0]` reference its results.json never contained, and the control in fact ran on the same machine in ten minutes) — the matched `<methods_registry>` sheet's "Cross-validation control pairs" section lists the field's standard pairs (perturbative ↔ full diagonalization, analytic limit ↔ numerics, MC ↔ closed form, second library). In Phase 3 you EXECUTE the control at one representative point and record `computed.cross_validation: [{"claim_key": "<computed.* key>", "method_a": ..., "method_b": ..., "value_a": <from an executed command>, "value_b": <ditto>, "tolerance_rel": <frozen now, before values exist>, "artifact": "<script path>"}]`. Additional tooth (2026-08-24): every `claim_key` in `cross_validation` and `cross_validation_plan` must name a `computed.*` leaf that EXISTS in results.json — the write tool validates on save and returns nearest-key suggestions on a miss; a control filed under a phantom key is credit lost at the join. Teeth you should know about: the finish gate recomputes the verdict from the two values itself (your verdict field is ignored), requires both values to appear in the harness job transcripts (a narrated number cannot anchor), and rejects method_a = method_b. If no independent method exists within scope, say so in the contract — the dependent claim then ships at `indicative` grade at best, with hedged wording. An executed disagreement is a FINDING (record it, don't tune it away); an unexecuted control is a GRADE CAP, not a failure.
...
- **Premise corrections (typed, 2026-08-25)**: if your computation PROVES a numeric or structural premise of the plan/task wrong (observed case: the plan said 297 nm ↔ n≈55–65; the computed mapping gives n≈75.3 — with C₆ ∝ n¹¹ consequences for every downstream experiment), do not park it in Limitations prose. Record it in results.json as `computed.premise_corrections: [{"premise": "<what the plan assumed>", "corrected": "<what you computed, with the number>", "consequence": "<one line: what physics/inputs change downstream>", "affects": ["E_3", "report"]}]`. The harness surfaces every unacknowledged entry to brain each turn as a forced replanning decision; a correction in prose reaches nobody. Limitations still gets the prose version for the reader.
- **Quantity declarations (claims-first, 2026-08-26)**: every quantitative deliverable is a project-wide QUANTITY. In this contract list `computed.quantities: [{"id": "<snake_case, project-wide identity>", "key": "computed.<leaf>"}]` — REUSE an id that already appears in `<claim_status>` when you re-estimate the same observable (the estimate histories must join; a near-duplicate id is rejected at write time with the existing id suggested). Predicates (`f40_gt_f10`) are not quantities: declare them as `computed.verdicts: [{"id": ..., "reads": ["<quantity ids it depends on>"], "replaces": {"<old read>": "<new read>"}}]` — dropping a read that an earlier experiment's same verdict had, with no `replaces`, blocks finish (the 297nm run silently swapped a decay channel this way). In Phase 3, for every quantity that will headline, fill in: `"headline": true`, `"observable"` (one sentence: what is measured, units, conditions — the reviewer's discriminator and the contradiction auditor read it), `"uncertainty"` (a NUMBER with `"uncertainty_source"`; a string is rejected, not coerced; without σ the quantity can never be corroborated), `"limit_check": {"limit", "expected", "observed", "artifact"}` (a limit with a KNOWN answer — a zero-expected limit discriminates nothing and counts only if the reviewer attests it), and `"inputs": {"<quantity id>": <value you actually used>}` (VALUES, not ids — two estimates reading the same input at different values are incomparable, and their disagreement is routed to that input).
- **Disputes are facts you record, not problems you resolve (2026-08-26)**: when a `cross_validation` entry comes out DISCREPANT, record it and move on — `cross_validation_resolved` no longer clears anything, and a paragraph explaining why your method is the authoritative one is exactly the move this rule exists to stop (the 297nm run "resolved" a 4.3× disagreement by declaring its own script authoritative; the number reached the abstract). Name in your `### FollowUp:` the DISCRIMINATING computation — the one whose result differs depending on which side is right. Only a third independent estimate, a blind replication, or a non-producer adjudication with a locator settles a dispute.
...
**Descriptions carry observable, inputs and invariants — never the predicted outcome.** A test named `test_f40_lt_f10_ordering` that asserts the acceptance criterion's prediction is confirmation bias with a green checkmark (observed, 297nm); the prediction lives in your frozen criterion, which `tool_review` does not see. `tool_impl` writes `scripts/<name>.py`; `tool_review` writes `tests/test_<name>.py`. Neither reads the other's output during its initial write — they work from the same description independently.
...
4. A section in `notes/experiments.md` under `## L2.X — <topic>` — **written by a `ledger_writer` sub-agent you spawn, not by you.** The interpretation-fidelity study located the dominant error class in exactly this turn: after 90 messages of context, producer models write "at most X / does not exist" from failed searches and drop recorded caveats. A fresh-context opus writer fed pinned facts is the measured fix. Spawn it as your LAST act of Phase 3, after results.json is final:
   ...
   - **Figure candidates:** one line per plottable artifact — `<runs/run_N/data/<file>> → <suggested plot type + the claim it would settle>; points: <N per axis>; sigma: <column name | none — why>`. **Figure-grade data (2026-08-29):** a sweep that will carry a headline figure has ≥ 20 points per control axis and a σ column (from the same computation that produced the value — a fit residual, a convergence delta, a method spread); four points and no σ is a table, not a figure, and brain will send the sweep back before it briefs the figure. Two controls → write the full grid, not the envelope. Brain's figure pass keys off this line; an artifact you don't list here is a figure that silently never gets made. If the experiment is genuinely scalar (single numbers — no scan, no comparison, no distribution), write `### No figure: <one-sentence rationale>` instead. Exactly one of the two MUST be present.
...
- **Open reviewer finding** (v3 D1): when a revision round's feedback is not fully addressed, the L2 section's `### Limitations` carries `finding_open: "<reviewer's first sentence verbatim>"`; when it is addressed, `finding_answered: <clause> — <locator>`. Optionally mirror it in results.json as `computed.reviewer_open_issues: ["…"]`. The finish gate reads these; a flaw that lives only in a spawn result is invisible to the referee.
...
improvement, 不可行, 排除, 开放问题, 无法; ALSO positive causal attributions: attributed to, due to, explains, consistent with our hypothesis, likely caused by, 归因, 表明, 源于, 可能是由):**
