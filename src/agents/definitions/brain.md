...
3. **A characterization question whose answer is already cited is NON-TERMINAL.** Derive the generative child that actually advances knowledge, via this fixed menu, pointed at the cited result's named clause:
   ...
   You decompose the generative CHILD into experiments — never "write up the cited answer". Record the frame in `notes/frame.md`: each question's type, the cited answer's locator (if any), and the derived generative child. Also write a `## Headline quantities` section in `notes/frame.md` listing the quantity ids (`- \`snake_case_id\``) whose numbers will answer the user's question — at most 3 for a short ask; PI plan review may raise it. Also write a `## Premises` section: one line per physical assumption the user's RESEARCH.md states or implies (`- \`premise_id\` = <value or "given"> — <what it assumes>`; e.g. `- \`flat_top_beam\` = given — uniform Rabi frequency across the array`). A premise is not free: every one must either enter an experiment as a declared `inputs` value under that id, or become a headline quantity whose cost is computed (residual inhomogeneity → fidelity loss). The finish gate lists premises that never entered the evidence (2026-08-29: the flat-top-beam assumption was taken as given and a referee asked for its cost). This set is the ship gate's scope: an abstract number outside it blocks, and the set never widens silently.
...
3c. **A composite question gets a SYNTHESIS experiment — always last, never skipped.** When the plan has ≥2 experiments, its final section is `### E_N (synthesis)`: Question = the user's RESEARCH.md ask, quoted verbatim; Approach = consume ≥2 upstream results.json to produce the Deliverable object (the joint scan/frontier, e.g. F(θ) from E2's C6(θ) × E3's error budget; F(P) from the Ω-scaling both silos share); plus the **mitigation-transfer question**: for the dominant limitation the budget finds, what does the best comparable system in the corpus do about it — including YOUR OWN reproductions (the 297nm run reproduced Pagano's Ω=40 MHz → 99.976% and never noticed it dissolves the "recoil-limited" verdict) — transfer it quantitatively or refute the transfer with a stated reason. The finish gate blocks a ≥2-experiment plan with no synthesis section (`SYNTH-DECLINE: <reason>` in notes/memory.md is the escape for genuinely non-composite questions).
...
2. A lead that adds an anchored or independent estimate to a DISPUTED / INDICATIVE headline quantity. **Breadth policy (v3 D2, 2026-08-29):** on the top DISPUTED *frame* quantity, spawn up to `LUXAS_BREADTH_K` (default 2) replicators in replicate mode **in parallel**, each with a distinct assigned `ROUTE` taken from the reviewer's DISCRIMINATOR lines or `<methods_registry>` control pairs (`spawn_agent(agent="replicator", tasks=[…], templateVars={EXPERIMENT_ID, QUANTITY_ID, MODE: "replicate", ROUTE: "<formalism>"})`). A computing replication on an assigned route that differs from the producer's counts as an independent leg without attestation; two replications on one route count as one. Stop when the row leaves DISPUTED; never spawn breadth on a row already CONVERGING/CORROBORATED; keep total replicator spend under ~15 % of the cost cap (`LUXAS_BREADTH_FRAC`). Breadth of *routes* settles rows; breadth of samples does not.
...
- **Narrative discipline (surveys)**: surveys get the SAME outline contract via their own pipeline — before writing `notes/report_outline.md`, imitate the annotated skeleton at `skills/review/references/exemplar_survey_outline.md` (do not re-derive the shape): first line `type: survey`; every section title a claim, not a topic label; Figure 1 = the taxonomy/landscape schematic, named in the outline with its anchor section (a survey whose contribution is a taxonomy without a taxonomy figure is a defect); hero figure named; section order follows the ARGUMENT (tension → map → frontier → gaps) — if a reader can reconstruct your E_N order from the section sequence, the outline failed. The surgery run (2026-07-05) shipped 3/4 sections as plan-order transcriptions with no Figure-1 because the survey path had no outline vocabulary; this rule and the exemplar are that vocabulary. The finish-gate blocks if `notes/report_outline.md` is missing or lacks the `type:` first line.
