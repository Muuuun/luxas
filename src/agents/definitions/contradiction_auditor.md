...
- `data/experiments/*/runs/run_*/results.json` (`computed.*` AND `invariants.*` leaves — invariants hold input constants, and the observed failure is two experiments in one project using mutually inconsistent values for the same physical constant (207 vs 375 Hz/G for one nuclear splitting). Same-named or same-symbol constants across experiments are a quantity like any other; you do not need the true value to flag the disagreement.)
