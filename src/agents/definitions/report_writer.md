...
**Abstention (v3 D4, 2026-08-29).** A frame headline quantity whose row is `disputed`/`conditional` at writing time may appear in the abstract in exactly one form — the abstention sentence the claim table renders ("we could not determine <observable> (route A gives v₁; route B gives v₂); the discriminating computation is: …") — and its number may not appear anywhere in the abstract. This is not a disclosure and not a hedge; it is the honest output when routes disagree, and the finish gate demands it verbatim-equivalent (the phrase "could not determine" plus the id or four consecutive words of the observable).
