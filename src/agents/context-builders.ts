...
${smartTruncate(method, 4000)}
  ...
  try {
    ...
    if (rendered) {
      parts.push(rendered);
      parts.push(`<pi_figure_sources>\nA data figure's editable source is data/experiments/<E>/figures/<name>.figspec.json (rendered by the strict figspec template); an energy-level diagram's is <name>.levelspec.json (rendered by levelspec); any other schematic's is its .tex. A missing plot_*.py is NOT a defect and must not be requested — illustrator_write cannot write matplotlib (refused at write time). Ask for spec changes: a panel tag (condition), group (one hue per species), role reference|envelope, linestyle, sigma, a where row filter, a shorter label, a dropped series, one highlight. A <pdf>.figlint.json with errors means the renderer refused the figure; the compile gate refuses it too until the spec is fixed. Figure audits: ≤ 2 rounds, fixes only for BLOCKING items.\n</pi_figure_sources>`);
