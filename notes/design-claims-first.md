...
**Second live run (pp-vs-ss, 2026-08-27, fixed code), E1 round 1:** producer fields on the entries 100% (`headline`/`observable`/`uncertainty`/`uncertainty_source`), `limit_check` 20%, `inputs` 50%; 3 blind replicators (cap held) + a replicate-mode run; reviewer DISCRIMINATOR + SCALING for 4/4 declared headline ids; blind estimates disputed two (sign of C₆^SS, sign+magnitude of C₆^PP(0°)); VERDICT revise. **`INDEPENDENT` / `ANCHOR-OK` attestations: 0 in both runs** — the reviewer never writes them unprompted, so no pair can ever reach `converging`/`corroborated`. Open: either the obligation block must demand them per pair explicitly, or the attestation requirement is demoted (design §3.3). Not demoted here — it gates corroboration.

**§3.7 build-order condition — met on the second live run (2026-08-27):** `claims_dispatch` verdict CHANGED: after the first dispute signal the brain dispatched 5 settling agents (blind replicators, discriminator experiment) and 0 cosmetic ones. The two disputes that survived round 2 were a θ=0 basis-convergence question (legitimate) and an n-scaling exponent 8.66 vs 11 near a Förster resonance (legitimate flag; the n¹¹ expectation is the thing that is wrong there). A third, C₆^SS, was a **sign-convention** disagreement (|−138.86| vs |+140|); the table now says so in the reason and names the one-round answer.
