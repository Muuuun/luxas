...
Correction (2026-08-30, Ba run): I first read the regenerated hero figure (serif, tab10-like palette) as a pgfplots bypass of the spec route; it was figspec output under the project's `physics-aps.mplstyle` (Computer Modern, `usetex`, six-colour cycle). The pgfplots refusal added to `figureSpecOnly` stays — it closes a real hole — but no bypass occurred. What the figure did expose: seven series on a six-colour cycle wrapped to an identical blue; the renderer now varies the line style on wrap and warns on stderr.
